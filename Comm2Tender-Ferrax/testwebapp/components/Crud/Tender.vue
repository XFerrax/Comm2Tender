...
import Form from '~/components/Form/Tender.vue'

var props = defineProps(mixinPropsCrud)
const useCrud = mixinUseCrud()

const $helpers = helpers()
const apiAddress = 'tender'
const itemKey = `${apiAddress}Id`
const title = 'Тендеры'
const titleSuffixForm = 'тендера'

const headers = ref([
  { title: 'ID тендера', key: itemKey, align: 'end', sortable: true, },
  { title: 'Com номер', key: 'number', align: 'start', sortable: true, },
  { title: 'Описание', key: 'discription', align: 'center', sortable: true, },
  { title: 'Контрагент победитель', key: 'winnerProposal', align: 'center', sortable: false, },
])
